param(
    [string]$Description = "dashboard deploy $(Get-Date -Format 'yyyy-MM-dd HH:mm')"
)

$ErrorActionPreference = 'Stop'
$repoRoot = Split-Path -Parent $MyInvocation.MyCommand.Path
$projectRoot = Join-Path $repoRoot 'IoT-Dashboard'
$log = Join-Path $repoRoot ("dashboard-deploy-{0}.log" -f (Get-Date -Format 'yyyyMMdd-HHmmss'))
...
if (-not $deploymentId) {
    throw 'IoT-Dashboard/.clasp.json does not contain deploymentId.'
}

Write-Log '=================================================='
Write-Log 'Dashboard - clasp deploy'
...
Write-Log '[OK] Dashboard deploy completed.'
