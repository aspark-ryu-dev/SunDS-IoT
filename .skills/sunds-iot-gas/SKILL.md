...
- raw key は `Readings` / `Latest` で変更しない。
- 多層の標準 key は `MetricMappings` で管理し、値は `CanonicalLatest` に分離する。
- 標準 key の先頭は `state` / `trigger` / `interval` / `cumulative` / `system` のいずれかにする。
- AI は POST の同期処理で呼び出さない。既知キーはローカルルール、未知キーだけをバックグラウンドで解析する。
- 会議室Nowは `Devices.location` を固定タグとして扱い、人数判定には `state.*` だけを使用する。
- 古い会議室プロジェクトは `sample/old now` の参照資料であり、旧DBや毎分全件再計算を移植しない。
